...
{ carrier := {f : polynomial R | f.nat_degree ≤ n},
  zero_mem' := zero_le n,
  add_mem' := λ p q hpn hqn, le_trans (nat_degree_add_le p q) (max_le hpn hqn),
  smul_mem' := λ c p hpn, le_trans (nat_degree_smul_le c p) hpn,
}
...
lemma ker_lcoeff (f : polynomial R) (n : ℕ) (hfn : f ∈ M R (n+1)) (hf : lcoeff R (n+1) f = 0) :
  f ∈ M R n :=
begin
  rw [mem_M_iff, nat_degree_le_iff_coeff_eq_zero] at hfn ⊢,
  intros i hni,
...
#check polynomial.nat_degree_X_pow
...
begin
  ...
  { exact le_of_eq (polynomial.nat_degree_X_pow _) },
end
