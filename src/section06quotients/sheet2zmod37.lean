...
-- This unlocks a bunch of notation
...
-- (this is handy to know; let's give it a name)
lemma equiv_def (a b : ℤ) : a ≈ b ↔ R a b :=
begin
  refl
end

-- quotient.mk is surjective
...
-- two integers are related iff their images are equal
...
except that this formula will depend on `a` (in the case of negation we
...
lemma negation_is_well_defined (a b : ℤ) (h : a ≈ b) : ⟦-a⟧ = ⟦-b⟧ :=
begin
  sorry,
end
...
def neg : Zmod37 → Zmod37 := quotient.lift (λ a, ⟦-a⟧) negation_is_well_defined
...
-- and the proof, and it will spit out a function `f` from `Y` to `α`.
