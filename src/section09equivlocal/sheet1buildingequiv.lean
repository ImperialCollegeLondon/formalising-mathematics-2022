...
{ to_fun := sorry, -- this is data; try to stay away from tactic mode
  inv_fun := sorry, -- ditto
  left_inv := begin
...
-- it says "this explicit function is a bijection from `X` to `Y` and this 
-- other explicit function is its inverse".
...
-- The equivalence classes of `R` are called *cardinals*.
